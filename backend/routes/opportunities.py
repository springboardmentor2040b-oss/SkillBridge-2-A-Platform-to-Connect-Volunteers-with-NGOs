...
from database import opportunities_collection, users_collection, ngos_collection
...
async def create_opportunity(opportunity: OpportunityCreate, current_user: dict = Depends(get_current_user)):
    if current_user.get("role") != "ngo" and current_user.get("role_in_ngo") not in ["owner", "admin"]:
        raise HTTPException(status_code=403, detail="Access denied: NGOs only")

    opp_data = opportunity.dict()
    opp_data["ngo_id"] = current_user.get("ngo_id") or str(current_user["id"])  # for backward compatibility
...
async def get_ngo_opportunities(current_user: dict = Depends(get_current_user)):
    ...
    cursor = opportunities_collection.find({"ngo_id": ngo_id}).sort("created_at", -1)
    opportunities = []
...
async def get_all_opportunities_for_ngo(current_user: dict = Depends(get_current_user)):
    if current_user.get("role") != "ngo" and current_user.get("role_in_ngo") not in ["owner", "admin"]:
        raise HTTPException(status_code=403, detail="Access denied: NGO owners/admins only")

    cursor = opportunities_collection.find({}).sort("created_at", -1)
    opportunities = []
    async for doc in cursor:
        doc["id"] = str(doc["_id"])
        ngo = await ngos_collection.find_one({"_id": ObjectId(doc["ngo_id"])})
        if ngo:
            doc["ngo_name"] = ngo.get("name")
            doc["organization_name"] = ngo.get("name")
        else:
            # for old opportunities, ngo_id is user_id
            user = await users_collection.find_one({"_id": ObjectId(doc["ngo_id"])})
            if user:
                doc["ngo_name"] = user.get("name")
                doc["organization_name"] = user.get("organization_name")
        opportunities.append(doc)
    return opportunities
