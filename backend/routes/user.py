...
async def register_user(user: UserRegister):
    ...
    if user.role == "ngo":
        # Create NGO
        ngo_data = {
            "name": user.organization_name or user.name,
            "owner_email": user.email,
            "created_at": datetime.utcnow()
        }
        ngo_result = await ngos_collection.insert_one(ngo_data)

        user_data["ngo_id"] = str(ngo_result.inserted_id)
        user_data["role_in_ngo"] = "owner"

    else:
        user_data["ngo_id"] = None
        user_data["role_in_ngo"] = None
    ...
    return UserResponse(
        ...
        role_in_ngo=user_data.get("role_in_ngo"),
...
async def login_user(user: UserLogin):
    ...
    existing_user.get('ngo_id'),
    existing_user.get('role_in_ngo')
)
    ...
    user_info = {
        ...
        "ngo_id": existing_user.get("ngo_id"),
        "role_in_ngo": existing_user.get("role_in_ngo"),
...
async def get_dashboard_stats(current_user: dict = Depends(get_current_user)):
    ...
    # Check if user is part of NGO (either role='ngo' or has ngo_id)
    if current_user.get("ngo_id") or current_user.get("role") == "ngo":
        # Use ngo_id if available (new system), fallback to user id (legacy)
        ngo_id = current_user.get("ngo_id") or str(current_user["id"])
        
        # Active Opportunities
        opp_count = await opportunities_collection.count_documents({"ngo_id": ngo_id, "status": "open"})
        
        # Pending Applications count
        # 1. Get all NGO opp IDs
        opp_cursor = opportunities_collection.find({"ngo_id": ngo_id})
        opp_ids = []
        ...
        volunteers_list = await applications_collection.distinct("volunteer_id", {"opportunity_id": {"$in": opp_ids}})
        unique_volunteers = len(volunteers_list) if volunteers_list else 0
        
        # Team members count
        team_members = await users_collection.count_documents({"ngo_id": ngo_id})
        
        stats = {
            "activeOpps": opp_count,
            "pendingApps": app_count,
            "totalVolunteers": unique_volunteers,
            "totalOpps": await opportunities_collection.count_documents({"ngo_id": ngo_id}),
